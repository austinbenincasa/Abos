...
static void irq_install_handler(int irq, void (*handler)(struct interrupt_struct *state))
{
    irq_handlers[irq] = handler;
}

//uninstalled a irq handler
static void irq_uninstall_handler(int irq)
{
    irq_handlers[irq] = 0;
}

static void irqs_install(void)
{
